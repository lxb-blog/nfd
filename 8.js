...
// 以下为各功能模块背景图片
const statusBgImage = 'https://img.siyouyun.eu.org/file/1740569053174_IMG_2363.png'; // 机器人状态背景图
const helpBgImage = 'https://img.siyouyun.eu.org/file/1740571550415_IMG_2365.png'; // 帮助菜单背景图
const blockBgImage = 'https://img.siyouyun.eu.org/file/1740568575434_IMG_2364.png'; // 屏蔽用户确认界面背景
const unblockBgImage = 'https://img.siyouyun.eu.org/file/1740557604080_p0 2.png'; // 解除屏蔽确认界面背景
const addFraudBgImage = 'https://img.siyouyun.eu.org/file/1740548062053_p0.png'; // 添加欺诈用户确认背景1236546
const removeFraudBgImage = 'https://img.siyouyun.eu.org/file/1740666584421_IMG_2398.png'; // 移除欺诈用户确认背景
const checkStatusBgImage = 'https://img.siyouyun.eu.org/file/1740666581688_IMG_2394.png'; // 检查状态背景图
const finalTextBgImage = 'https://img.siyouyun.eu.org/file/1740666584991_IMG_2397.png'; // 欺诈列表背景图
const handleUserInfoBgImage = 'https://img.siyouyun.eu.org/file/1740666583692_IMG_2395.png'; // 查询用户信息背景图
const handleBlockListBgImage = 'https://img.siyouyun.eu.org/file/1740666589433_IMG_2396.png'; // 查询被屏蔽用户列表背景图
...
async function onCallbackQuery(callbackQuery) {
 ...
 // 在 switch (action) 下的 case 分支中修改如下：
      case 'cancel_block':
      case 'cancel_unblock':
  ...
  if (message.text && message.text.startsWith('/') && message.text !== '/start') {
    if (message.chat.id.toString() !== ADMIN_UID) {
      ...
      const { result } = await sendMessage({
        chat_id: message.chat.id,
        text: '⛔ 该指令仅主人可用' // 修改提示避免用户感知重复
      });

      // 通过 Event Loop 机制锁定时序
      await new Promise(resolve => setTimeout(resolve, 480)); // 关键延迟
      await deleteMessage({
...
async function handleUnfraudByReply(message) {
  ...
  if (!existing) {
    return sendPhoto({
      chat_id: ADMIN_UID,
      photo: removeFraudBgImage, // 可以使用提示错误专用的图片
